...
await page.evaluate(() => window.map.setView([-21.30, 55.62], 16));
...
const target = await page.evaluate(async () => {
  ...
  const params = new URLSearchParams({
    ...
    bbox, count: '20', outputFormat: 'application/json', srsName: 'EPSG:4326'
  });
...
const selectedCount = await page.evaluate(() => window.selectedParcels?.size ?? '(no window.selectedParcels)');
