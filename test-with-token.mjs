...
if (!TOKEN) { console.error("usage: node test-with-token.mjs <dbx_token>"); process.exit(1); }
...
await page.evaluate(t => { localStorage.setItem('dbx_token', t); }, TOKEN);
...
console.log('\n=== console (relevant) ===');
logs.filter(l => /dbx|culture|crop|err|warn/i.test(l)).slice(-15).forEach(l => console.log(l));
